...
# ⚠️ REEMPLAZA CON LA URL DE TU GOOGLE SHEETS
...
if enviado:
    if not documento or not correo or not celular:
        ...
        try:
            # 1. Convertir la URL normal en una URL de exportación directa en formato CSV
            # Especificamos el nombre de la hoja usando &sheet=
            nombre_hoja = "Listado de aprendices"
            base_url = URL_GOOGLE_SHEETS.split("/edit")[0]
            csv_url = f"{base_url}/export?format=csv&sheet={nombre_hoja}"
            
            # 2. Leer la hoja usando Pandas de forma nativa e infalible
            ...
            # 3. Buscar coincidencia en la columna L
            ...
            if coincidencia_encontrada:
                # 4. Para guardar usando la conexión estándar de Streamlit sin el bug del lector:
                from streamlit_gsheets import GSheetsConnection
                conn = st.connection("gsheets", type=GSheetsConnection)
                
                # Al pasarle los datos directo al update evitamos que use el 'read' problemático
                conn.update(spreadsheet=URL_GOOGLE_SHEETS, sheet=nombre_hoja, data=df)
